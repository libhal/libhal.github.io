...
libhal uses **C++20 coroutines** as the primary mechanism for asynchronous
...
In many cases, the usage of `co_await` or `co_return` is not necessary and adds
...
- Reading a sample from an ADC with continuous conversion
  - Continuous conversion means that the ADC is always sampling the analog pin
...
hal::future<hal::u16> my_adc::driver_read(async::context& p_ctx) {
  // If this resource is already in use by another context, then `lock` will
  // suspend the context until the resource becomes available. When the guard
  // is destroyed, p_ctx's ownership of this resource is released.
...
extern "C" {
// This ISR just has to unblock the context and that's it
...
The same pattern works even if your interrupt is more complicated. For example,
...
its work. In these cases, suspending for a duration of time is useful.

Use `co_await async::yield(p_ctx)` or time-based blocking to periodically check
status:
...
Choose a time delay if you have an expectation of when the data will be
